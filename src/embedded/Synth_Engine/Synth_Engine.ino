...
static byte WaveFormTable[MaxWaveTypes][Sample_num] = {
  // Sin wave
  {
...
void setup() {
  ...
  timerAlarmWrite(Timer0_Cfg, 50, true);
...
void loop() {
  ...
  //playFreq(40);
 // if (OutBufferIndex - FillBufferIndex > 100) {
    for(int a = 0; a < 100; a++) {
      FillBufferIndex++;
      if (FillBufferIndex >= Sample_rate) FillBufferIndex = 0;
      molt = modf((FillBufferIndex * freq / (double)Sample_rate), &intPart);
      val = molt * Sample_num;
      wave[FillBufferIndex] = WaveFormTable[0][val];
    }
  }
// }
